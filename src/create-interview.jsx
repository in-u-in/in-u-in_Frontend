...
import styles from './styles/create-interview.module.css'
...
export default function CreateInterviewPage () {
    ...
    useEffect(() => {
        const validateForm = () => {
            if (title && step && purpose && category && takenTime && defaultPrice && extraPrice &&
                !titleError && !stepError && !purposeError && !categoryError && !takenTimeError && defaultPriceError && extraPriceError) {
                setFormValid(true);
            } else {
                setFormValid(false);
            }
        ...
        validateForm();
    }, [title, step, purpose, category, takenTime, defaultPrice, extraPrice,
        titleError, stepError, purposeError, categoryError, takenTimeError, defaultPriceError, extraPriceError]);
            ...
            <div className={styles.InputContainer}>
                <form onSubmit={handleSubmit}>
                    <div className={styles.InputBox}>
                        ...
                        <input className={styles.Input1} type="text" placeholder="제목을 입력하세요" onChange={onTitleChange}/>
                    ...
                    <div className={styles.InputBox}>
                        ...
                        {purposeError && <div className={styles.ErrorMessage}>{purposeError}</div>}
                    </div>
                    <div className={styles.InputBox}>
                        <div className={styles.SubTitle}>
                            <div className={styles.Star}>*</div>
                    ...
                    <div className={styles.InputBox}>
                        ...
                        <input className={styles.Input2} type="text" onChange={onTakenTimeChange} placeholder="분"/>
                        {takenTimeError && <div className={styles.ErrorMessage}>{takenTimeError}</div>}
                    </div>
                    ...
                    <div className={styles.InputBox}>
                        ...
                        <input className={styles.Input2} type="text" onChange={onDefaultPriceChange} placeholder="원"/>
                        {defaultPriceError && <div className={styles.ErrorMessage}>{defaultPriceError}</div>}
                    </div>
                    <div className={styles.InputBox}>
                        <div className={styles.SubTitle}>
                            <div className={styles.Star}>*</div>
                        ...
                        <input className={styles.Input2} type="text" onChange={onExtraPriceChange} placeholder="원"/>
                        {extraPriceError && <div className={styles.ErrorMessage}>{extraPriceError}</div>}
                    </div>
                    <div className={styles.InputBox}>
                        <div className={styles.Title2}>모집 메인글 작성</div>
                        <input className={styles.Input3} type="text"/>
                    </div>
                    <div type= "submit" className={styles.CreateButton}>인터뷰 생성</div>
                </form>
            </div>
        </div>
    )
